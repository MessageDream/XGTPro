//
//  CustomSwipTableCellTableViewCell.m
//  LightErector
//
...
@interface CustomSwipeTableInputOverlay : UIView
@property (nonatomic, weak) CustomSwipeTableCell * currentCell;
@end

@implementation CustomSwipeTableInputOverlay
...
@interface CustomSwipeButtonsView : UIView
@property (nonatomic, weak) CustomSwipeTableCell * cell;
@end

@implementation CustomSwipeButtonsView
{
    NSArray * buttons;
    UIView * container;
    BOOL fromLeft;
...
-(instancetype) initWithButtons:(NSArray*) buttonsArray direction:(CustomSwipeDirection) direction
{
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, maxSize.width * buttonsArray.count, maxSize.height)]) {
        fromLeft = direction == CustomSwipeDirectionLeftToRight;
...
#pragma clang diagnostic ignored "-Wundeclared-selector"
    if ([sender respondsToSelector:@selector(callCustomSwipeConvenienceCallback:)]) {
        //call convenience block callback if exits (usage of CustomSwipeButton class is not compulsory)
        autoHide = [sender performSelector:@selector(callCustomSwipeConvenienceCallback:) withObject:_cell];
    }
#pragma clang diagnostic pop
    ...
    if (_cell.delegate && [_cell.delegate respondsToSelector:@selector(swipeTableCell:tappedButtonAtIndex:direction:fromExpansion:)]) {
        ...
        autoHide|= [_cell.delegate swipeTableCell:_cell tappedButtonAtIndex:index direction:fromLeft ? CustomSwipeDirectionLeftToRight : CustomSwipeDirectionRightToLeft fromExpansion:fromExpansion];
    }
...
-(void) transition:(CustomSwipeTransition) mode percent:(CGFloat) t
{
    switch (mode) {
        case CustomSwipeTransitionStatic: [self transitionStatic:t]; break;
        case CustomSwipeTransitionDrag: [self transitionDrag:t]; break;
        case CustomSwipeTransitionClipCenter: [self transitionClip:t]; break;
        case CustomSwipeTransitionBorder: [self transtitionFloatBorder:t]; break;
        case CustomSwipeTransition3D: [self transition3D:t]; break;
    }
}
...
@implementation CustomSwipeSettings
-(instancetype) init
{
    if (self = [super init]) {
        self.transition = CustomSwipeTransitionBorder;
        self.threshold = 0.5;
    }
    return self;
}
...
@implementation CustomSwipeExpansionSettings
-(instancetype) init
{
    if (self = [super init]) {
        self.buttonIndex = -1;
...
typedef struct CustomSwipeAnimationData {
    CGFloat from;
    CGFloat to;
    CFTimeInterval duration;
    CFTimeInterval start;
} CustomSwipeAnimationData;


#pragma mark CustomSwipeTableCell Implementation

@implementation CustomSwipeTableCell
{
    ...
    UIView * swipeOverlay;
    UIView * swipeView;
    CustomSwipeButtonsView * leftView;
    CustomSwipeButtonsView * rightView;
    bool allowSwipeRightToLeft;
    bool allowSwipeLeftToRight;
    __weak CustomSwipeButtonsView * activeExpansion;
    
    CustomSwipeTableInputOverlay * tableInputOverlay;
    __weak UITableView * cachedParentTable;
    UITableViewCellSelectionStyle previusSelectionStyle;
    
    CustomSwipeAnimationData animationData;
...
-(void) initViews
{
    ...
    _leftSwipeSettings = [[CustomSwipeSettings alloc] init];
    _rightSwipeSettings = [[CustomSwipeSettings alloc] init];
    _leftExpansion = [[CustomSwipeExpansionSettings alloc] init];
    _rightExpansion = [[CustomSwipeExpansionSettings alloc] init];
...
-(void) fetchButtonsIfNeeded
{
    if (_leftButtons.count == 0 && _delegate && [_delegate respondsToSelector:@selector(swipeTableCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)]) {
        _leftButtons = [_delegate swipeTableCell:self swipeButtonsForDirection:CustomSwipeDirectionLeftToRight swipeSettings:_leftSwipeSettings expansionSettings:_leftExpansion];
    }
    if (_rightButtons.count == 0 && _delegate && [_delegate respondsToSelector:@selector(swipeTableCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)]) {
        _rightButtons = [_delegate swipeTableCell:self swipeButtonsForDirection:CustomSwipeDirectionRightToLeft swipeSettings:_rightSwipeSettings expansionSettings:_rightExpansion];
    }
}
...
-(void) createSwipeViewIfNeeded
{
    ...
    if (!leftView && _leftButtons.count > 0) {
        leftView = [[CustomSwipeButtonsView alloc] initWithButtons:_leftButtons direction:CustomSwipeDirectionLeftToRight];
    ...
    if (!rightView && _rightButtons.count > 0) {
        rightView = [[CustomSwipeButtonsView alloc] initWithButtons:_rightButtons direction:CustomSwipeDirectionRightToLeft];
...
- (void) addInputOverlayIfNeeded
{
    ...
    tableInputOverlay = [[CustomSwipeTableInputOverlay alloc] initWithFrame:table.bounds];
...
- (void)setSwipeOffset:(CGFloat) newOffset;
{
    ...
    CustomSwipeButtonsView * activeButtons = sign < 0 ? rightView : leftView;
    ...
    CustomSwipeButtonsView* but[2] = {leftView, rightView};
    CustomSwipeSettings* settings[2] = {_leftSwipeSettings, _rightSwipeSettings};
    CustomSwipeExpansionSettings * expansions[2] = {_leftExpansion, _rightExpansion};
    
    for (int i = 0; i< 2; ++i) {
        CustomSwipeButtonsView * view = but[i];
        if (!view) continue;
...
-(void) showSwipe: (CustomSwipeDirection) direction animated: (BOOL) animated
{
    ...
    UIView * buttonsView = direction == CustomSwipeDirectionLeftToRight ? leftView : rightView;
    
    if (buttonsView) {
        CGFloat s = direction == CustomSwipeDirectionLeftToRight ? 1.0 : -1.0;
...
-(void) panHandler: (UIPanGestureRecognizer *)gesture
{
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        CustomSwipeButtonsView * expansion = activeExpansion;
        if (expansion) {
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if (gestureRecognizer == panRecognizer) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(swipeTableCell:canSwipe:)]) {
            allowSwipeLeftToRight = [_delegate swipeTableCell:self canSwipe:CustomSwipeDirectionLeftToRight];
            allowSwipeRightToLeft = [_delegate swipeTableCell:self canSwipe:CustomSwipeDirectionRightToLeft];
        }
        else {
            [self fetchButtonsIfNeeded];
