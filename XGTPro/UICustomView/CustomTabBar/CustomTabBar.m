...
- (void)buildUI {
    ...
    [self cleanTabView];
    //build UI
    for (CustomTabBarItem *item in self.tabItems) {
                ...
                switch (tabItem.tabState) {
                    case TabStateDisabled:
                        if ([self delegateRespondsToDisableSelector]) {
                            [self.delegate tabBar:self tabBecameDisabledAtIndex:[self indexOfTab:tabItem] tab:tabItem];
                        }
                        break;
                    case TabStateEnabled:
                        if ([self delegateRespondsToEnableSelector]) {
                            [self.delegate tabBar:self tabBecameEnabledAtIndex:[self indexOfTab:tabItem] tab:tabItem];
                        }
                        break;
                }
            }
...
- (void)pressedTab:(id)sender {
    ...
    CustomTabBarItem *tabItem = [self tabItemForTab:tabView];
    [self swtichTab:tabItem];
}

#pragma mark - Helper methods

- (UIControl *)existingTabForTabItem:(CustomTabBarItem *)tabItem {
...
- (int)indexOfTab:(CustomTabBarItem *)tabItem {
    return [self.tabItems indexOfObject:tabItem];
}

- (CustomTabBarItem *)tabItemForTab:(UIControl *)tab {
    return self.tabItems[[self.tabViews indexOfObject:tab]];
}

- (CGRect)frameForTab:(CustomTabBarItem *)tabItem {
...
- (void)setTabContent:(UIControl *)tab withTabItem:(CustomTabBarItem *)tabItem {
...
- (void)setTabContent:(CustomTabBarItem *)tabItem {
    UIControl *tab = [self tabForItem:tabItem];
    [self setTabContent:tab withTabItem:tabItem];
}

- (UIControl *)tabForItem:(CustomTabBarItem *)tabItem {
...
- (BOOL)delegateRespondsToDisableSelector {
    if ([self.delegate respondsToSelector:@selector(tabBar:tabBecameDisabledAtIndex:tab:)]) {
        return YES;
    } else {
...
- (BOOL)delegateRespondsToEnableSelector {
    if ([self.delegate respondsToSelector:@selector(tabBar:tabBecameEnabledAtIndex:tab:)]) {
        return YES;
    } else {
