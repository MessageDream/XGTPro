...
@implementation UIScrollView (CustomPullRefreshView)
...
{
    CustomPullRefreshView *view = [[CustomPullRefreshView alloc] initWithImage:[UIImage imageNamed:@"centerIcon"]
                                                          position:position];
    switch (view.position) {
        case CustomPullRefreshViewPositionTop:
        case CustomPullRefreshViewPositionBottom:
...
@interface CustomPullRefreshViewBackgroundLayer : CALayer
...
@implementation CustomPullRefreshViewBackgroundLayer
...
/*-----------------------------------------------------------------*/
@interface CustomPullRefreshView()

@property (nonatomic, assign) BOOL isUserAction;
@property (nonatomic, assign) BOOL isObserving;
@property (nonatomic, assign) CustomPullRefreshViewState state;
...
@property (nonatomic, strong) CustomPullRefreshViewBackgroundLayer *backgroundLayer;
...
@implementation CustomPullRefreshView
...
- (void)_commonInit
{
    ...
    self.state = CustomPullRefreshViewStateNormal;
    ...
    CustomPullRefreshViewBackgroundLayer *backgroundLayer = [[CustomPullRefreshViewBackgroundLayer alloc] initWithBorderWidth:self.borderWidth];
...
- (void)setupScrollViewContentInsetForLoadingIndicator:(actionHandler)handler
{
    ...
    if (self.position == CustomPullRefreshViewPositionTop) {
...
- (void)setProgress:(double)progress
{
    ...
    if (progress >= 0 && progress <= 1.0f) {
        ...
        animationImage.fromValue = [NSNumber numberWithFloat:DEGREES_TO_RADIANS(135+180*self.prevProgress)];
        animationImage.toValue = [NSNumber numberWithFloat:DEGREES_TO_RADIANS(135+180*progress)];
...
- (BOOL)isSidePosition
{
    return (self.position == CustomPullRefreshViewPositionLeft || self.position == CustomPullRefreshViewPositionRight);
}
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset
{
    ...
    switch (self.state) {
        case CustomPullRefreshViewStateNormal: //detect action
            if (self.isUserAction && !self.scrollView.dragging && !self.scrollView.isZooming && self.prevProgress > 0.99f) {
                [self actionTriggeredState];
            }
            break;
        case CustomPullRefreshViewStateStopped: // finish
        case CustomPullRefreshViewStateLoading: // wait until stopIndicatorAnimation
            break;
        default:
            break;
    }
...
{
    self.state = CustomPullRefreshViewStateLoading;
